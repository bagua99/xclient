...
function M:getLocalSeat(ser_seat)
	...
	local curValue = (ser_seat + G_GameDefine.GAME_PLAYER - self.main_player.seat) % G_GameDefine.GAME_PLAYER + 1
	return curValue
end
...
function M:getServerSeat(nLocalSeat)
	...
	local curValue = (nLocalSeat + self.main_player.seat - 1) % G_GameDefine.GAME_PLAYER
	return curValue ~= 0 and curValue or G_GameDefine.GAME_PLAYER
end
