...
local GameDeskLayer             = require("app.scenes.gamedesk."..GameConfigManager.tGameID.YZBP..".game.GameDeskLayer")
local GameEndLayer              = require("app.scenes.gamedesk."..GameConfigManager.tGameID.YZBP..".game.GameEndLayer")
local GameTotalEndLayer         = require("app.scenes.gamedesk."..GameConfigManager.tGameID.YZBP..".game.GameTotalEndLayer")
local logic                     = require("app.scenes.gamedesk."..GameConfigManager.tGameID.YZBP..".logic.logic")
local GameCardManager           = require("app.scenes.gamedesk."..GameConfigManager.tGameID.YZBP..".card.GameCardManager")
...
function M:initView()
    ...
    -- 投降
    self.SurrenderLayer = GameLeaveLayer.create()
    self.SurrenderLayer:setVisible(false)
    self:addChild(self.SurrenderLayer)
end
...
function M:handleMessage(name, msg)
    ...
    elseif name == "yzbp.GAME_PlayerEnterAck" then
		self:handlePlayerEnterAck(msg)
    elseif name == "yzbp.GAME_PlayerLeaveAck" then
		self:handlePlayerLeaveAck(msg)
    elseif name == "yzbp.GAME_EnterGameAck" then
		self:handleEnterGameAck(msg)
	elseif name == "yzbp.GAME_GameSceneAck" then
		self:handleSceneAck(msg)
    elseif name == "yzbp.GAME_ReadyAck" then
		self:handleReadyAck(msg)
	elseif name == "yzbp.GAME_GameStartAck" then
        self:handleGameStartAck(msg)
    elseif name == "yzbp.GAME_CallScoreAck" then
        self:handleCallScoreAck(msg)
    elseif name == "yzbp.GAME_MainCardAck" then
        self:handleMainCardAck(msg)
    elseif name == "yzbp.GAME_SendBackCardAck" then
        self:handleSendBackCardAck(msg)
    elseif name == "yzbp.GAME_SurrenderAck" then
        self:handleSurrenderAck(msg)
    elseif name == "yzbp.GAME_SurrenderVoteAck" then
        self:handleSurrenderVoteAck(msg)
    elseif name == "yzbp.GAME_SurrenderVoteResultAck" then
        self:handleSurrenderVoteResultAck(msg)
    elseif name == "yzbp.GAME_BuryCardAck" then
		self:handleBuryCardAck(msg)
	elseif name == "yzbp.GAME_OutCardAck" then
		self:handleOutCardAck(msg)
    elseif name == "yzbp.GAME_PromptAck" then
		self:handlePromptAck(msg)
	elseif name == "yzbp.GAME_GameEndAck" then
		self:handleGameEndAck(msg)
    elseif name == "yzbp.GAME_GameTotalEndAck" then
...
function M:handleEnterGameAck(msg)
    ...
    -- 玩家人数
    G_GameDefine.player_count = self.tRoomInfo.player_count
...
function M:handleGameStartAck(msg)
    -- 游戏叫分状态
    G_GameDefine.nGameStatus = G_GameDefine.game_score
    -- 设置游戏局数
    G_GameDefine.nGameCount = G_GameDefine.nGameCount + 1

    -- 隐藏结算界面
    if self.pGameEnd ~= nil then
	    self.pGameEnd:removeFromParent()
    end
    self.pGameEnd = nil
...
function M:handleCallScoreAck(msg)
    if msg.nNextCallScore == G_GameDefine.invalid_seat then
        -- 游戏叫主状态
        G_GameDefine.nGameStatus = G_GameDefine.game_main_card
    end
...
function M:handleMainCardAck(msg)
    -- 游戏埋底状态
    G_GameDefine.nGameStatus = G_GameDefine.game_bury_card

    -- 桌子层消息处理
    self.GameDeskLayer:handleMainCardAck(msg)
end

-- 发送底牌消息
function M:handleSendBackCardAck(msg)
    -- 桌子层消息处理
    self.GameDeskLayer:handleSendBackCardAck(msg)
end

-- 投降消息
function M:handleSurrenderAck(msg)
    -- 游戏埋底状态
    G_GameDefine.nGameStatus = G_GameDefine.game_surrender

    -- 桌子层消息处理
    self.GameDeskLayer:handleSurrenderAck(msg)
end

-- 投降投票消息
function M:handleSurrenderVoteAck(msg)
    -- 桌子层消息处理
    self.GameDeskLayer:handleSurrenderVoteAck(msg)
end

-- 投降结果投票消息
function M:handleSurrenderVoteResultAck(msg)
    -- 桌子层消息处理
    self.GameDeskLayer:handleSurrenderVoteResultAck(msg)
end
...
function M:handleBuryCardAck(msg)
    ...
    -- 桌子层消息处理
    self.GameDeskLayer:handleBuryCardAck(msg)
end
...
function M:handleGameEndAck(msg)
    ...
    local function showEnd()
        ...
        self:runAction(cc.Sequence:create(cc.DelayTime:create(3.0), cc.CallFunc:create(function()
            if self.pGameEnd == nil then
                self.pGameEnd = GameEndLayer.create()
                self:addChild(self.pGameEnd)
                self.pGameEnd:GameEndAck(G_GameDefine.nGameCount, G_GameDefine.nTotalGameCount, msg)
            else
                self.pGameEnd:GameEndAck(G_GameDefine.nGameCount, G_GameDefine.nTotalGameCount, msg)
            end
            -- 解散不显示小局结算
            if self.bDisovleGame then
                self.pGameEnd:setVisible(false)
            end
        end)))
    end
...
function M:handleGameTotalEndAck(msg)
    if self.pGameGameTotalEnd == nil then
        self.pGameGameTotalEnd = GameTotalEndLayer:create()
        self.pGameGameTotalEnd:GameTotalEndAck(msg, self.bDisovleGame, self.tRoomInfo.master_id, G_Data.roomid)
        self.pGameGameTotalEnd:setVisible(false)
        self:addChild(self.pGameGameTotalEnd, 101)
    end
...
function M:handlePromptAck(msg)
    G_CommonFunc:showGeneralTips(GameConfigManager.tGameID.YZBP, msg.szPrompt, self,cc.p(display.cx,display.cy))
end
...
function M:showGameTotalEnd()
    ...
    if self.pGameGameTotalEnd ~= nil then
        self.pGameGameTotalEnd:setVisible(true)
    end
end
